...
vec4 getGlitchColor(vec2 uv, float force) {
  vec2 r = vec2(
    random(vec2(ceil(time * 1.0), 0.0)),
    random(vec2(0.0, ceil(time * 1.0)))
  );
  vec2 noiseUv = uv + r * 0.001;
  float mask = smoothstep(
    length(vec3(1.0)),
    length(vec3(1.0)),
    length(texture2D(textureNoise, uv * r).rgb)
    );
  vec4 texColor = texture2D(texture, noiseUv + r * 0.01 * force) * (1.0 - mask);
  vec4 texColorDiff = texture2D(texture, noiseUv + r * force) * .0;
  return texColor + texColorDiff;
}

void main() {
  float shake = random(vec2(time));
  float force = smoothstep(0.2, 5.0, sin(time * 4.0) * 0.8 + sin(time * 5.0) + 0.2);

  vec2 uvR = vUv + vec2(0.0, 0.0);
  vec2 uvG = vUv + vec2( 0.0, 0.0);
  vec2 uvB = vUv + vec2( 0.003 + shake * 0.002, 0.0);
